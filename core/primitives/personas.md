...
Flags usability risks involving ARIA attributes, screen reader behavior, focus management, contrast ratios, target sizes, and keyboard navigation. Recommendations are advisory unless the task or applicable standard makes them acceptance criteria. Load `core/standards/interaction-design.md` for the rule set.

**Coordinator profile**
Check output against `core/agents.md` Core Contract and the relevant standard (`core/standards/implementation.md` for code, `core/standards/prose.md` for text).
...
**UI/UX designer**
Evaluates visual hierarchy, information flow, spacing, color usage, and interaction patterns. Focuses on whether the tool "reads" correctly: do users' eyes flow from input to output naturally? Load `core/standards/interaction-design.md` for the rule set.

**Visual consistency auditor**
Pixel-level design system compliance. Audits every font size, weight, color, radius, and gap against the documented system. Catches drift and undocumented one-off values. Load `core/standards/interaction-design.md` for the rule set.
...
**Chaos Monkey QA**
Walks through every reachable state as a user, computes pixel budgets at mobile viewports (375px/390px/768px), tests edge cases visually rather than by code inspection. Load `core/standards/interaction-design.md` for the rule set.
