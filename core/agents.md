...
## Languages & Stack
- Primary: Python, C++
- Python tooling: pip, flake8, black
- Use type hints in all Python code; prefer `typing` module for complex types
...
- **Project-level primitives.** Primitives are not limited to this file. A given project, repository, or working directory may define its own (reviewer personas, agent configurations, skill definitions, prompt templates, workflow files). Discover and incorporate them the same way: select the relevant ones, compose them with the primitives defined here, and adapt. Project-level primitives take precedence over global ones when they conflict, since they carry more specific context.
...
- Functions should return values, not null/void — enable chaining and composition
...
## Workflow Rules
- **Apply this profile to all generated code**: Every preference in this file is a hard requirement, not optional guidance. Re-read before writing new files. Check compliance before presenting code.
...
- **Estimate before starting.** Read the request and assign a tier:
  - **T1 — trivial** (<5 minutes): typo, one-line change, obvious bug. Skip planning, just do it.
...
## Copy Style
- **No em dashes** in UI copy or comments. Use periods, commas, or conjunctions.
...
- **Domain accuracy**: Use terminology the target audience uses. Consult domain experts (e.g., the pottery panel) for wording decisions. Avoid engineering jargon in user-facing text.
