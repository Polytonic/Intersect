...
How the preferences reach each CLI and how the pieces relate:
...
graph TD
    ...
    Claude -- ~/.claude/CLAUDE.md --> CW
    Codex -- ~/.codex/AGENTS.md --> AG
    Gemini -- ~/.gemini/GEMINI.md --> AG
    CW --> AG
    CW --> P
    AG --> P
    AG --> PL
...
Claude goes through `core/claude.md` because its CLI does not reliably expand transitive `@imports`. Codex and Gemini read `agents.md` directly.
