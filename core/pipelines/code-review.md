...
- **Agent prompt construction**: Brief each persona per `core/primitives/coordination.md` Briefing Protocol. Include the relevant style file (`core/styles/implementation.md` for code review) so agents check against the actual coding rules. Same shared context for all agents, distinct focus lens for each.
